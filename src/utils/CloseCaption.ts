import { Call, type CallClosedCaption, type ClosedCaptionEvent } from '@stream-io/video-client'
...
export class ClosedCaptionManager {
  ...
  renderToggleElement() {
    ...
    button.textContent =
      this.status === 'on' ? 'Turn off closed captions' : 'Turn on closed captions'
  ...
  showCaptions() {
    this.status = 'on'
    this.unsubscribe = this.call.on('call.closed_caption', (event: ClosedCaptionEvent) => {
      const caption = event.closed_caption
      const isDuplicate = this.captions.find(
        (c) => c.speaker_id === caption.speaker_id && c.start_time === caption.start_time,
      )
      if (!isDuplicate) {
        const speaker = this.call.state.participants.find((p) => p.userId === caption.speaker_id)
  ...
  private updateDisplayedCaptions() {
    if (!this.captionContainer) {
      console.warn('Render caption container before turning on closed captions')
      return
    }
    const displayedCaptions = this.captions.slice(-1 * this.numberOfCaptionsVisible)
