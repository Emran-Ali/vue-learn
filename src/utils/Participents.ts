...
const renderScreenShare = (
  ...
  if (hasScreenShare(participant)) {
    ...
    if (!screenEl) {
      ...
      // registers subscription updates and stream changes
      const unbind = call.bindVideoElement(screenEl, participant.sessionId, 'screenShareTrack')
  ...
  if (hasScreenShareAudio(participant)) {
    ...
    if (!audioEl) {
      ...
      const unbind = call.bindAudioElement(audioEl, participant.sessionId, 'screenShareAudioTrack')
...
export const cleanupParticipant = (sessionId: string) => {
  ...
  const unbundScreenShareAudio = audioBindingsCache.get(`screen-audio-${sessionId}`)
